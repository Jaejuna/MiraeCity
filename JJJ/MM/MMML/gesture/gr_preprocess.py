...
def process_image(filename):  # 이미지를 처리하는 부분을 함수로 분리합니다
    # 파일의 절대 경로를 만듭니다
...
for root, _, filenames in tqdm(os.walk(data_root), desc="Processing images"):  # tqdm을 사용하여 진행 막대를 표시합니다
    for i in range(0, len(filenames), batch_size):  # 배치 크기만큼 이미지를 처리합니다
        batch = filenames[i:i + batch_size]
        for filename in batch:
            df = process_image(filename)
            if df is not None:
                dfs.append(df)
